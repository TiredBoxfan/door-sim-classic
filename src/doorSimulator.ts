...
import { delay, gameprompt, getch, keyp, stringOut, systemPause, userInput, wordCheckB, wordCheckP, wordOrder } from "./basicFunctions.js";
...
let logging: boolean = true; //For Logging things
		...
		*/
		...
		do{
			...
			else if (wordCheckB("inventory"))
			{
				if (sword == true)
				{
					write("You have a broadsword, capable of felling a whole enemy if need be the occasion\n");
				}
				else
				{
					write("You have a small song of good feelings.\n");
					write("You also have a signed copy of 'DOORS AND YOU: A BEGINNER'S GUIDE TO DOOR OPENING'");
				}
			...
			else if (userInput == "killthelich")
			{
				...
				write("You feel a chill go through your body, as if some terrible evil is upon you.\n");
				setTextColor("#0f0");
				gamestyle = 1;
			}
				...
				if ((wordCheckB("sword")))
				{
					if (gamestyle == 1)
					{
						...
						if ((wordCheckB("handle")) || (wordCheckB("hilt")))
						{
							if (sword == false)
							{
								write("You have picked up the sword, it is heavy yet swinging it feels almost effortless.\n");
								sword = true;
							}
							else
							{
								write("You already have a sword, why replace your nice one with the non existent one \nthat you seem to think is on the floor?\n");
							}
						}
						else
						{
							write("You grab the sword blade and cut yourself accidentally,\nYou have dropped the sword.\nYour hand is now bleeding.\n");
						}
					}
					else
					{
			...
			else if (wordCheckB("timetoslam"))
			{
				if (iceproof == false)
				{
					if (gamestyle == 0)
					{
						...
						await stringOut("Theres a time and a place for everything, 111but not now.\n");
					}
					else
					{
						write("Wind starts forming a small twister within the room, you hear a crowd \ncheering as the count down\n");
						await keyp();
						await delay(1000);
						write("5\n\n");
						await delay(1000);
						...
						write("2\n\n");
						await delay(1000);
						write("Just then as if by some spectral magic, Charles Barkley jumps out from\n"
						...
						write("Charles looks to you and forms a new ball in his hands from the\n"
							+"spectral winds of the twister.  He then yells\n");
						await keyp();
						await stringOut("\t'SPECTRAL SLAM OF ICE PROTECTION'\n");
						await keyp();
						write("and in a magnificent display of wind and lightning,\n"
				...
				else
				{
					...
					await stringOut("\t'There is a time and place for everything\n \tbut not now.'\n");
				}
			...
			else if (wordCheckB("search"))
			{
				...
				else
				{
					if (sword == false)
					{
						write("There is a sword on the floor and a small inscription on the wall.\n");
					}
					else
					{
			...
			else if ((wordOrder("open", "door")) || (wordOrder("pull", "handle")))
			{
				switch(handstate)
				{
					case(-1):
					{
						write("You pull your hand away from the door though the door does not follow suit.\nYour hand is no longer on the door.\n");
						handstate = 0;
						break;
					}
					case(0):
					{
						write("Though your efforts vailant, you are unable to open the door through sheer willpower alone.\nIt feels as if you are missing some vital steps here.\n");
						break;
					}
					
					case(1):
					{
						write("The door resists as if some strange mechanism was holding it shut\nIt feels as if you are missing some vital steps here.\n");
						break;
					}
					
					case(2):
					{
						write("The door resists as if there was an opposing force, \ndesigned to hold said door shut.\n");
						break;
					}
					case(3):
					{
						if (stepback == false)
						{
							write("The door comes towards you full force and unfortunately, \nyour face was in the path of it.\n"
						...
						{
							clear();
							write("The door opens and nearly misses your face as you pull it inwards\n"
			...
			{
				write("Turns taken: "+turnstaken+"\n"
					+"Doors opened: "+gamestyle+"\n"
					+"Maidens saved: 0"+"\n"
					+"Liches killed: 0"+"\n"
					+"Current alignment: ");
				if (left == true)
			...
			else if ((userInput.includes("step")) || (userInput.includes("walk")))
			{
                ...
                if (userInput.includes("back") && 0 < wordCheckP("back"))
				{
					...
					switch(direction)
					{
						case(0):
						{
							...
							{
								write("There is a wall there that you so gracefully collide with.\n");
							}
							break;
						}
						case(1):
						{
							write("The room is at such a small size that such an action is impossible.\n");
							break;
						}
						case(2):
						{
							...
							{
								write("You collide with the door as you back up...\nNope still not open.\n");
							}
							break;
						}
						case(3):
						{
							write("You attempt such a feat but there is a wall behind you.\nbetter luck next time.\n");
							break;
						}
                ...
                else if (userInput.includes("forward") && 0 < wordCheckP("forward"))
				{
					...
					switch(direction)
					{
						case(0):
						{
							...
							{
								write("You would move closer to the door but you dont want to crowd it.\n");
							}
							break;
						}
						case(1):
						{
							write("You start to walk towards the wall but unfortunately the wall enforces its physical being on your foot.\nYou now have hurt your foot.\n");
							break;
						}
						case(2):
						{
						...
						case(3):
						{
							write("The wall kindly reminds you of its existence as you collide with it.\n");
							break;
						}
                ...
                else if (userInput.includes("left") && 0 < wordCheckP("left"))
				{
					...
					switch(direction)
					{
						case(0):
						{
								write("The wall kindly rejects your offer of affection for it by colliding with you quite rudely.\n");
						...
						case(1):
						{
							...
							{
								write("You find that the door meets your physical contact with little to no reaction.\n");
							}
							break;
						}
						case(2):
						{
							write("You find that the walls persist in their stance of keeping the room together\n");
							break;
						}
						case(3):
						{
							if (stepback == false)
							{
								write("You shuffle to the leftward of your current positon,\naway from the glorious Door.\n");
								stepback = true;
							}
							else
							{
								write("You offer the argument of your movement but unfortunately the walls have voted \nto persist.\n");
							}
							break;
						}
					}
                ...
                else if (userInput.includes("right") && 0 < wordCheckP("right"))
				{
					switch(direction)
					{
						case(0):
						{
							write("There is a wall there that decides its existence is more\nimportant than your progression through it.\n");
							break;
						}
						case(1):
						{
							if (stepback == false)
							{
								write("Right you go, away from any door shaped entities in the area.");
								stepback = true;
							}
							else
							{
								write("Walls are hard to pass through in your current physical form\n");
							}
							break;
						}
						case(2):
						{
							write("You slide so far to the right that in fact you haven't moved at all.\nMostly because of physical barriers such as walls.\n");
							break;
						}
						case(3):
						{
							if (stepback == true)
							{
								write("You make your way right in search of the door.\nYou have achieved this goal");
								stepback = false;
							}
							else
							{
			...
			else if (wordCheckB("sing"))
			{
				...
				const song = "Of dangers and dragons and Liches and more1\nthere was never a challenge as fierce as the door.\n\n11Through legends and myth, with many to try,\n1the door was never the one who would die.\n\n11So for those who dare, to challenge its might,\n1just be sure to turn the knob right111\n\nor left.\n";
				await stringOut(song);
			
			}
			//Grabbing
			
			else if (((userInput.includes("grab")) || (userInput.includes("put")) || (userInput.includes("place")) || (userInput.includes("establish")) || (userInput.includes("lay")) || (userInput.includes("settle"))|| (userInput.includes("establish")) || ((userInput.includes("grasp"))) || (userInput.includes("hold"))) && !((userInput.includes("sword")) || userInput.includes("hilt")))
			{
			
			
				if (userInput.includes("hand") && 0 < wordCheckP("hand") || ((userInput.includes("grasp"))) || (userInput.includes("hold")) || (userInput.includes("grab")))
				{
					if ((wordOrder("hand", "door") || ((userInput.includes("grasp"))) || (userInput.includes("hold")) && ( userInput.includes("door"))) || ( userInput.includes("knob")) || ( userInput.includes("handle")))
					{
						if ( (userInput.includes("knob")) || ( userInput.includes("handle"))) // old one, mysteriously evaluates to true always(((wordOrder("door", "knob")) &&( userInput.includes("knob"))) || ((userInput.includes("grasp")) && ( userInput.includes("knob"))) || ((userInput.includes("hold")) && ( userInput.includes("knob"))) || ((userInput.find("door")) <userInput.find("knob")) || (((userInput.includes("grasp"))) &&( userInput.includes("handle"))) || ((userInput.includes("hold")) &&( userInput.includes("handle"))))
						{
							...
							{
								write("You gently place your hand on the door knob\n"
						...
						else
						{
							if (handstate == -1)
							{
								write("I understand your compulsive need to put your hand more on the door\n"
									+"but your hand is about as on the door as it gets.\n");
							}
							else
							{
								handstate = -1;
								write("Your hand is now placed upon the door.\n"
			...
			else if ((userInput.includes("turn")) || (userInput.includes("rotate")))
			{
				...
				if (((wordOrder("turn", "knob") && ( userInput.includes("knob"))) || (wordOrder("turn", "handle") && ( userInput.includes("handle")))) || (wordOrder("rotate", "knob") && ( userInput.includes("knob"))) || (wordOrder("rotate", "handle") && ( userInput.includes("handle"))))
				{
					if ((wordOrder("knob", "left") && ( userInput.includes("left"))) || (wordOrder("knob", "right") && ( userInput.includes("right"))) || (wordOrder("handle", "left") && ( userInput.includes("left"))) || (wordOrder("handle", "right") && ( userInput.includes("right"))))
					{
						...
						switch(handstate)
						{
							...
							case(1):
							{
								write("You attempt to rotate the door knob but you lose your gentle grip\n"
							...
							case(3):
							{
								...
								if (userInput.includes("left"))
								{
									if (left == true)
									{
										write("You cannot turn the doorknob any more left than it already is.\n");
										break;
									}
									else if (right == true)
									{
								...
								else if (userInput.includes("right"))
								{
									...
									if (left == true)
									{
										write("The doorknob has now returned to its unturned state.\n");
										left = false;
										handstate = 2;
										break;
									}
									...
									{
										write("Trust me, it is not right to turn the knob right any more.\n");
										break;
									}
					...
					else
					{
						switch(handstate)
						{
							...
							case(2):
							{
								write("Your hand rotates, but not in a direction that advances your quest to open a door\n");
								break;
							}
				...
				else if (wordOrder("turn", "left"))
				{
					...
					switch(direction)
					{
						case(0):
						{
							write("You turn left to face door, the door is now in front of you again.\n");
							break;
						}
						case(1):
						{
							write("You turn left to face the wall, the door is now on your left.\n");
							break;
						}
						case(2):
						{
							write("You turn left to face the back wall, the door is now behind you.\n");
							break;
						}
						case(3):
						{
							write("You turn left to face the wall, the door is now on your right.\n");
							break;
						}
				...
				else if ((wordOrder("turn", "right")) && userInput.includes("right"))
				{
					...
					switch(direction)
					{
						case(0):
						{
							write("You turn right to face door, the door is now in front of you again.\n");
							break;
						}
						case(1):
						{
							write("You turn right to face the wall, the door is now on your left.\n");
							break;
						}
						case(2):
						{
							write("You turn right to face the back wall, the door is now behind you.\n");
							break;
						}
						case(3):
						{
							write("You turn right to face the wall, the door is now on your right.\n");
							break;
						}
		...
		if (gamestyle == 0)
		{
			...
			write("\n"
                ...
                clear();
				write("                         A new day brings new adventure,\n"
					+"                                 but for now...\n\n\n"
					+"                                 rest easy heroes\n\n\n\n\n\n");
				await systemPause();
                clear();
...
{
	clear();
	write("A cold breeze blows across your face as you open the door.\n"
		+"It is the Lich!\n"
	...
	do
	{
		...
		if ((userInput.includes("shut") || userInput.includes("close") || userInput.includes("slam")) && userInput.includes("door"))
			{
				write("You slam the door in the Lich's face and see ice cover the entire inner portion of the door\n"
		...
		else if ((userInput.includes("step") ) || (userInput.includes("walk") ))
		{
			...
			else if (iceoff == true)
			{
				...
				write("The Lich looks upon you confused at what you were attempting when suddenly\n"
					+"You hear a voice.>>\n");
					await getch();
				...
				write(">>\n");
					await getch();
				write("You begin to thaw but the Lich realizes and quickly stops it using more\n"
		...
		else if (userInput.includes("timetoslam"))
		{
			...
			{
				write("You begin to shout \n'TIME TO' \nJust then, the Lich freezes you.\n");
				write("Just then, you hear a voice.\n"
					+"'Its not the slam in your game\n"
					+"its the slam in your heart'\n");
				write("You muster all of your strength to shout the final word\n"
					+"'SLAM'\n"
				...
				write("he shouts at Charles as he crumbles to the ground.\n"
			...
			if (userInput.includes("sword"))
			{
				if (userInput.includes("lich"))
				{
					if (FinalAttack == false)
					{
						if (iceoff == false)
						{
							write("You begin to strike the Lich but he unfortunately freezes you before you can \nland your blow.\n"
								+"Better luck next time\n"
								+"Game Over.\n"
						...
						{
							write("You begin to strike as the Lich freezes you solid with your blade mid-swing\n"
							...
							write(">>\n");
							await getch();
							write("The Lich is too busy laughing to notice that you are out of the ice\n"
					...
					{
						write("The Lich looks up to see not the icecube of a man which he expected,\n"
				...
				write("You swing your sword wildly but unfortunately not at the Lich\n"
		...
		{
			write("I said one chance and you couldn't even input a valid command?\n"
